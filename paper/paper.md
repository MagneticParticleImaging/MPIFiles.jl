...
Tomographic imaging methods allows to visualize the inner of the human body in a
non-invasive way. Most prominent examples used for medical diagnosis in medical
hospitals are the computed tomography (CT) and magnetic resonance imaging (MRI).
...
as organ perfusion. The MNP are harmless for the human body and are degraded in the
liver.

When working with an MPI tomograph there are various datasets involved. During an
...
The software package ``MPIFiles.jl`` provides an interface to all those types of
MPI data. In particular it has full support for the Magnetic Particle Imaging
...
``MPIFiles.jl`` supports the following type of MPI data
* measurements
* calibration data, i.e. system matrices
* reconstruction results
In addition to a low-level access to the data, the package also features
high level routines that to apply various post-processing methods such as
...
specification such that the later can be used as a reference for determining
the content and dimensionality of certain parameters.
``MPIFiles.jl`` is licensed under MIT licence and is hosted on Github

...
export Waveform, WAVEFORM_SINE, WAVEFORM_SQUARE, WAVEFORM_TRIANGLE, WAVEFORM_SAWTOOTH_RISING,
       ...
       rxNumChannels, rxNumSamplingPoints, rxNumSamplesPerPeriod, rxChannels, fieldChannels,
...
fieldChannels(field::MagneticField) = field.channels
...
electricalTxChannels(field::MagneticField) = [channel for channel in fieldChannels(field) if typeof(channel) <: ElectricalTxChannel]
electricalTxChannels(sequence::Sequence)::Vector{ElectricalTxChannel} = [channel for field in fields(sequence) for channel in fieldChannels(field) if typeof(channel) <: ElectricalTxChannel]
mechanicalTxChannels(field::MagneticField) = [channel for channel in fieldChannels(field) if typeof(channel) <: MechanicalTxChannel]
mechanicalTxChannels(sequence::Sequence)::Vector{MechanicalTxChannel} = [channel for field in fields(sequence) for channel in fieldChannels(field) if typeof(channel) <: MechanicalTxChannel]
periodicElectricalTxChannels(field::MagneticField) = [channel for channel in fieldChannels(field) if typeof(channel) <: PeriodicElectricalChannel]
periodicElectricalTxChannels(sequence::Sequence)::Vector{PeriodicElectricalChannel} = [channel for field in fields(sequence) for channel in fieldChannels(field) if typeof(channel) <: PeriodicElectricalChannel]
acyclicElectricalTxChannels(field::MagneticField) = [channel for channel in fieldChannels(field) if typeof(channel) <: AcyclicElectricalTxChannel]
acyclicElectricalTxChannels(sequence::Sequence)::Vector{ElectricalTxChannel} = [channel for field in fields(sequence) for channel in fieldChannels(field) if typeof(channel) <: AcyclicElectricalTxChannel]
...
function dfStrength(sequence::Sequence) # TODO: How do we integrate the mechanical channels and non-periodic channels and sweeps?
  channels = [channel for field in fields(sequence) for channel in fieldChannels(field) if typeof(channel) <: PeriodicElectricalChannel]
  maxComponents = maximum([length(channel.components) for channel in channels])
